...
public class CameraControls : MonoBehaviour {
	...
	//maximum size of the hex tile space map
	...
	void Update () {
				...
				///<summary> Panning mode: used to view overall hex tile map in space.</summary>
				if (mode == CameraMode.Panning) {
						// **Camera settings, initial position, and restrictions **
				...
				if(Input.touchCount == 2 ) {
					cameraHeight /= Pow(touchScroll(),3.0);
						...
						// **Selecting objects while in camera mode**
				...
				///<summary> Planet mode: camera rotates around the planet to view ground hex map for a specific planet.</summary>
			...
			if(Input.touchCount == 2 ) {
				rotateRadius /= Pow(touchScroll(),3.0);
						...
						//scroll away from planet to exit planet camera mode
						if (rotateRadius > 3.5f)
								mode = CameraMode.Returning;
						if (rotateRadius < minRotateRadius)
								rotateRadius = minRotateRadius;
		...
		///<summary>Move mode: used to select units; activates the GUI with specific action options</summary>
		...
		///<summary>Returning mode: Used to move back from a planet.</summary>
		if( mode == CameraMode.Returning ) {
			...
			//revert to panning mode when arrived at desired position
